...
Paral.result <- foreach(lambda.ind=1:M.paral, .combine='rbind') %do% {
  ...
  g <- ProxAlg_DFGM(estimated.cov.pc.X, estimated.cov.pc.Y, p, M.selected, lambda.choose)
  ...
  temp.result <- matrix(c(TPR, FPR), nrow=1)
  colnames(temp.result) <- c("TPR", "FPR")
  temp.result
}
stopCluster(cl)

save(Paral.result, file=paste(save.path, "/Para_result_p", p, "_runind", run.ind, ".RData", sep=""))
