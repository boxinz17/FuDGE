...
sub_obj <- function(W, B, lambda1, lambda2, rho){
  ...
  part3 <- (lambda2/rho)*(frobenius.norm(W[[1]]-W[[2]]))
...
sub_ADMM <- function(B, lambda1, lambda2, rho, tol.abs=1e-3, tol.rel=1e-3){
  ...
  while((!ind.primal)|(!ind.dual)){
    ...
    D1 <- W1_new + V1_old
    D2 <- W2_new + V2_old
    fnorm <- frobenius.norm(D1-D2)
    if (fnorm<=(2*lambda2/(rho*rho.prime))){
      R1_new <- 0.5*(D1+D2)
      R2_new <- 0.5*(D1+D2)
    } else {
      R1_new <- D1 - ((lambda2/(rho*rho.prime))/fnorm)*(D1-D2)
      R2_new <- D2 + ((lambda2/(rho*rho.prime))/fnorm)*(D1-D2)
    }
  ...
  for (i in 1:p){
    for (j in 1:p){
      if (i != j){
        ...
        Pen <- Pen + lambda2*frobenius.norm(sub.mat1-sub.mat2)
      } else {
        sub.mat1 <- Theta1[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)]
        sub.mat2 <- Theta2[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)]
        Pen <- Pen + lambda2*frobenius.norm(sub.mat1-sub.mat2)
...
FFGL_ADMM <- function(S, n.sam, p, M, lambda1, lambda2, tol.abs=1e-2, tol.rel=1e-3, mu=10, 
  ...
  while((!ind.primal)|(!ind.dual)){
    ...
    # (i) update Theta
    mat.temp <- S1 - (rho/n1)*Z1_old + (rho/n1)*U1_old
    eigen.res <- eigen(mat.temp)
    D <- diag(eigen.res$values)
    V <- eigen.res$vectors
    
    D.tilde <- D
    for (j in 1:dim(D)[1]){
      D.tilde[j,j] <- (n1/(2*rho))*(-D[j,j]+sqrt(D[j,j]^2+4*rho/n1))
    }
    
    Theta1_new <- V %*% D.tilde %*% t(V)
    
    mat.temp <- S2 - (rho/n2)*Z2_old + (rho/n2)*U2_old
    eigen.res <- eigen(mat.temp)
    D <- diag(eigen.res$values)
    V <- eigen.res$vectors
    ...
    D.tilde <- D
    for (j in 1:dim(D)[1]){
      D.tilde[j,j] <- (n2/(2*rho))*(-D[j,j]+sqrt(D[j,j]^2+4*rho/n2))
    }
    
    Theta2_new <- V %*% D.tilde %*% t(V)
    ...
    for (i in 1:p){
      for (j in 1:p){
        ...
        if (i == j){
          dist.fnorm <- frobenius.norm(B1-B2)
          if (dist.fnorm<=(2*lambda2/rho)){
            Z1_new[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)] <- 0.5*(B1+B2)
            Z2_new[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)] <- 0.5*(B1+B2)
          } else {
            Z1_new[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)] <- B1 - 
              ((lambda2*rho)/dist.fnorm)*(B1-B2)
            Z2_new[((i-1)*M+1):(i*M), ((j-1)*M+1):(j*M)] <- B2 + 
              ((lambda2*rho)/dist.fnorm)*(B1-B2)
          }
        } else {
          B <- list(B1, B2)
    ...
    #diff1 <- abs(obj.val.new-obj.val.old)/(abs(obj.val.old)+1e-18)
    #diff2 <- max(frobenius.norm(Theta1_new-Z1_new), frobenius.norm(Theta2_new-Z2_new))
    #print(diff2)
...
# S <- list(S1, S2)
# 
# res <- FFGL_ADMM(S, n.sam, p, M, lambda1, lambda2)
