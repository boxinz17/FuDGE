...
ProxAlg_DFGM <- function(cov.X, cov.Y, p, M, lambda, Eta="Auto", n.iteration=2000, Intialization=matrix(0, nrow=(p*M), ncol=(p*M)), e_tol=1e-2){
  ...
  grad.Mat <- cov.X %*% Delta.old %*% cov.Y - (cov.Y - cov.X)
  for (t in 1:n.iteration){
    ...
    grad.norm <- frobenius.norm(grad.Mat)
    grad.norms <- c(grad.norms, grad.norm)
    ...
    if ( grad.norm < e_tol){
      ...
      return (list(DelatMathat=Delta.hat, blockFrob=Delta.frob, converge=converge.indicator, 
                   num.iter=t, grad.norms=grad.norms))
      break
    }
    Delta.old <- Delta.new
  }
