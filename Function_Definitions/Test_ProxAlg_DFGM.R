...
source("E:/学习工作/硕士/graduate research/code/Differential Functional Graphical Model/Function_Definitions/ProxAlg_DFGM.R")
...
temp.result <- matrix(NA, nrow=length(lambda.v), ncol=2)
colnames(temp.result) <- c("TPR", "FPR")
count <- 0
for (lam in lambda.v){
  count <- count + 1
  ...
  g <- ProxAlg_DFGM(cov.X=estimated.cov.pc.X, cov.Y=estimated.cov.pc.Y, p=p, M=M.selected, 
  ...
  temp.result[count, 1] <- TPR
  temp.result[count, 2] <- FPR
}

plot(temp.result[, 2], temp.result[, 1], xlab="FPR", ylab="TPR", main="ROC Curve", type="b")
