...
for (i in 1:length(p.v)){
  ...
  png(file=paste(save.path, "/ROC_Model1_p", p, ".png", sep=""), width=600, height=600)
  plot(c(0, 0), xlim=c(0,1), ylim=c(0,1), xlab="False Postive Rate", type="n", 
       ylab="True Postive Rate", main=paste("Model1 p=", p, sep=""), cex.main=2.5, cex.lab=1.5, mai=c(1,1,1,1), 
       cex.axis=2)
...
write.table(AUC.table, file=paste(save.path, "AUC_tabel_Model1.txt", sep="/"))
write.csv(round(AUC.table, 2), file=paste(save.path, 
                                           "AUC_tabel_Model1(readable).csv", sep="/"))
