...
save.path <- "~/FuDGE/FuDGE_nbasis_test/result"
model.path <- "~/FuDGE/Model_Setting/Model_FuDGE_nabsis"
...
load(paste(model.path, "/model1_p", p, "_m", m, ".RData", sep=""))
Theta.L <- g
...
M.selected <- 4 # fix M to always be 4
...
Paral.result <- list(ROC.result, M.selected)

save(Paral.result, file=paste(save.path, "/Para_result_m", m, "_runind", run.ind, ".RData", sep=""))