...
Paral.result <- foreach(lambda2.ind=1:M.paral, .combine='rbind') %do% {
  timestart2 <- proc.time()
  
  lambda2 <- lambda2.v[lambda2.ind]
  print(paste("lambda2 is:", lambda2))
  
  JGL.result <- FFGL2_ADMM(cov.list, n.sam, p, M.selected, lambda1.choose, lambda2)
  ROC.res <- ROC_JGL2(JGL.result, Theta.L$SupportDelta, p, M.selected)
  ROC.point <- ROC.res$ROC.point
  
  print(paste("finished, iter num:", JGL.result$num.iter))
  timeend2 <- proc.time()
  time.use2 <- as.numeric((timeend2-timestart2)[3])
  print(paste("time used: ", time.use2, "(s)", sep=""))
...
Paral.result <- rbind(matrix(c(0,0), nrow=1), Paral.result)
Paral.result <- rbind(Paral.result, matrix(c(1,1), nrow=1))

ind.v <- rep(FALSE, dim(Paral.result)[1])
while(sum(!ind.v)>0){
  ind.v <- rep(TRUE, dim(Paral.result)[1])
  for(i in 2:dim(Paral.result)[1]){
    if (Paral.result[i, 1]<Paral.result[i-1, 1] | Paral.result[i, 2]<Paral.result[i-1, 2]){
      ind.v[i] <- FALSE
    }
  }
  Paral.result <- Paral.result[ind.v, ]
}
