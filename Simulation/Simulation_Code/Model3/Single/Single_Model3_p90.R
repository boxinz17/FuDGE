...
gam.v <- c(seq(0, 0.1, 0.02), seq(0.11, 0.2, 0.04), seq(0.4, 2, 0.8))
n.gam <- length(gam.v)
...
Paral.result <- foreach(gam.ind=1:n.gam) %do% {
  gX <- ProxAlg_FGM(estimated.cov.pc.X, p, M.selected, gam=gam.v[gam.ind], Eta=1e-3)
  gY <- ProxAlg_FGM(estimated.cov.pc.Y, p, M.selected, gam=gam.v[gam.ind], Eta=1e-3)
