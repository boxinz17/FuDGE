...
gam.v <- c(seq(0, 0.1, 0.005), seq(0.11, 0.2, 0.01), seq(0.4, 2, 0.2))
n.gam <- length(gam.v)
...
################### Part 3: Estimate the two graphs seperately #########################
Paral.result <- foreach(gam.ind=1:n.gam) %do% {
  gX <- ProxAlg_FGM(estimated.cov.pc.X, p, M.selected, gam=gam.v[gam.ind], Eta=1e-2)
  gY <- ProxAlg_FGM(estimated.cov.pc.Y, p, M.selected, gam=gam.v[gam.ind], Eta=1e-2)
  
  result.list <- list(gX=gX, gY=gY, gamma=gam.v[gam.ind])
  result.list
}
...
# save result
# save result
Otherinfo <- list(SX=estimated.cov.pc.X, SY=estimated.cov.pc.Y, Theta.L=Theta.L, gam.v=gam.v, 
                  M.selected=M.selected)

save(Paral.result, 
     file=paste(save.path, "/ThetaEst_Model1_p", p, "_runind", run.ind, ".RData", sep=""))
save(Otherinfo, 
     file=paste(save.path, "/Otherinfo_Model1_p", p, "_runind", run.ind, ".RData", sep=""))
